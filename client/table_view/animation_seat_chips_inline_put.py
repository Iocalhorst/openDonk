import time
class AnimationSeatChipsInlinePut():
      def __init__(self,player_info_widget,seat_widget,value):
          ...
          self.finished=False
          self.pending=True
          self.origin_rel_x=seat_widget.rel_x
          self.origin_rel_y=seat_widget.rel_y
          self.target_rel_x=seat_widget.chips_inline_rel_x
          self.target_rel_y=seat_widget.chips_inline_rel_y
      ...
      def start_now(self):
          assert self.is_pending()==True,"failed to start inline put animation. is_pending() != True - maybe you tried starting twice?! "
